    ...
    pub fn hud_settings(&mut self, ui: &mut Ui) {
        scroll(ui, "hud", |ui| {
            ...
            ui.collapsing("Minimap", |ui| {
                if checkbox(ui, "Enabled", &mut self.config.hud.minimap.enabled) {
                    self.send_config_game();
                }
                ...
                if checkbox(
                    ui,
                    "Center On Self",
                    &mut self.config.hud.minimap.center_on_self,
                ) {
                    self.send_config_game();
                }
                ...
                if drag(
                    ui,
                    "Zoom",
                    DragValue::new(&mut self.config.hud.minimap.zoom)
                        .range(1.0..=4.0)
                        .speed(0.02)
                        .max_decimals(2),
                ) {
                ...
                if drag(
                    ui,
                    "Marker Size",
                    DragValue::new(&mut self.config.hud.minimap.marker_size)
                        .range(3.0..=20.0)
                        .speed(0.1)
                        .max_decimals(1),
                ) {
                ...
                if checkbox_hover(
                    ui,
                    "Hide Native Minimap",
                    "Covers CS2's own top-left radar with a solid circle. Calibrated for 2560x1440 - may not align at other resolutions.",
                    &mut self.config.hud.minimap.hide_native_minimap,
                ) {
                    self.send_config_game();
                }
